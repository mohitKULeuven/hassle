...
def learn_from_random_model(
    n, clause_length, num_hard, num_soft, contexts_count, num_literal, model_seed
):
    ...
    param = f"_model_seed_{model_seed}_num_hard_{num_hard}_num_soft_{num_soft}_n_{n}__clause_length_{clause_length}_context_count_{contexts_count}_num_literal_{num_literal}"
    row = [
        model_seed,
        num_hard,
        num_soft,
